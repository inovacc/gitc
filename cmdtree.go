...
func gitcTree() cmdNode { //nolint:funlen // static command catalog
	...
	fetchFlags := []cmdFlag{
		...
		{Name: "full", Type: "bool", Description: "install the full git (bash + sh; the default); persists"},
		{Name: "minimal", Type: "bool", Description: "install the shell-less MinGit (smallest; no hooks); persists"},
	...
	native := []cmdNode{
		...
		{Name: "doctor", Usage: "git doctor", Short: "Health-check the install, backend, PATH shim, and audit DB"},
		{
