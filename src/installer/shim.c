...
// Vendored from https://github.com/71/scoop-better-shimexe (SPDX: MIT OR
// Unlicense). This C source is NOT compiled by the Go toolchain (the build
// constraint above excludes it, which also stops CGO_ENABLED=0 from rejecting a
// stray .c file); it is cross-compiled to the committed shim_<arch>.exe via
// `task shims` and embedded by embed.go. A tiny launcher: on start it reads a
// sibling `<name>.shim`
// (`path = <target>` + optional `args = ...`), then execs the target with
// those args plus its own forwarded argv, under a kill-on-close job object and
// with Ctrl+C forwarded to the child. gitc installs it as git/sh/bash.exe so
// each name is a ~15 KB launcher into one canonical gitc.exe, instead of a
// 15 MB per-name copy.
//
// Local change vs upstream: add <share.h> so `_SH_DENYNO` resolves under the
// mingw/UCRT toolchain used to cross-compile (MSVC exposes it via <stdio.h>).
